...
if [ ! -x "${CBSD_CMD}" ]; then
	...
	rootfs_url="https://www.bsdstore.ru/downloads/xigma/${majorver}/cbsd-14.0.9a.pkg"
...
TTYD_CMD=$( which ttyd 2>/dev/null )
[ ! -x "${TTYD_CMD}" ] && err 1 "No such ttyd executable"
...
EXTENSION_DIR="${EXTENSION_ROOT}/cbsd-jail"

# Make and move into the install staging folder
[ ! -d "${EXTENSION_ROOT}" ] && mkdir -p "${EXTENSION_ROOT}"
if [ ! -d ${START_FOLDER}/install_stage/cbsd-jail ]; then
	mkdir -p ${START_FOLDER}/install_stage/cbsd-jail || err 1 "ERROR: Could not create staging directory!"
fi
cd ${START_FOLDER}/install_stage/cbsd-jail || err 1 "ERROR: Could not access staging directory!"

echo "Retrieving the cbsdjail as a zip file"
fetch https://github.com/cbsd/xigmajail/archive/main.zip || err 1 "ERROR: Could not write to install directory!"
...
if [ ! -d "${EXTENSION_DIR}"} ]; then
	...
	/usr/local/bin/rsync -avz ${START_FOLDER}/install_stage/cbsd-jail/ ${EXTENSION_DIR}/
	...
	echo "Congratulations! The CBSD-jail extension was installed. Navigate to rudimentary config tab and push Save."
...
[ -d "${START_FOLDER}/install_stage/cbsd-jail" ] && rm -rf "${START_FOLDER}/install_stage/cbsd-jail"
rmdir ${START_FOLDER}/install_stage || true

CURRENTDATE=`date -j +"%Y-%m-%d %H:%M:%S"`
logger "[$CURRENTDATE]: jail installer!: installer: ${ACTION_MSG} successfully"
